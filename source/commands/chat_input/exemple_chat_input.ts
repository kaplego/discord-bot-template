...
export default {
		...
		options: [
			{
				...
				name: 'autocomplete',
				nameLocalizations: commandLocales.option('autocomplete').get('name'),
				description: 'Une option avec autocomplétition.',
				descriptionLocalizations: commandLocales.option('autocomplete').get('description'),
				autocomplete: true,
			},
		],
	},
	execute(interaction) {
		interaction.reply({
			// Renvoyer un embed
			embeds: [new Discord.EmbedBuilder().setTitle('Voilà').setDescription('Un exemple de commande slash.')],
			// Avec un bouton
			components: [
