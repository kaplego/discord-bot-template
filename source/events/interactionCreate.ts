import type { InteractionTypeCommand, InteractionTypeComponent } from '../types/bot_data';
...
export default {
	...
	listener([interaction], client) {
		...
		if (interaction.isCommand()) {
			...
			if (interaction.isChatInputCommand() && ChatInputs.has(interaction.commandName)) {
				interactionLogString += ':Slash';
				ChatInputs.get(interaction.commandName).execute(interaction, client);
			}
			// Si la commande est un menu contextuel de message
			else if (interaction.isMessageContextMenuCommand() && MessageActions.get(interaction.commandName)) {
				interactionLogString += ':Message';
				MessageActions.get(interaction.commandName).execute(interaction, client);
			}
			// Si la commande est un menu contextuel d'utilisateur
			else if (interaction.isUserContextMenuCommand() && UserActions.get(interaction.commandName)) {
				interactionLogString += ':User';
				UserActions.get(interaction.commandName).execute(interaction, client);
			}
			// Si la commande est une commande de tchat PRIVÉE
			else if (interaction.isChatInputCommand() && PrivateChatInputs.has(interaction.commandName)) {
				interactionLogString += ':Slash (PRIV)';
				PrivateChatInputs.get(interaction.commandName).execute(interaction, client);
			}
			// Si la commande est un menu contextuel de message PRIVÉE
			else if (interaction.isMessageContextMenuCommand() && PrivateMessageActions.get(interaction.commandName)) {
				interactionLogString += ':Message (PRIV)';
				PrivateMessageActions.get(interaction.commandName).execute(interaction, client);
			}
			// Si la commande est un menu contextuel d'utilisateur PRIVÉE
			else if (interaction.isUserContextMenuCommand() && PrivateUserActions.get(interaction.commandName)) {
				interactionLogString += ':User (PRIV)';
				PrivateUserActions.get(interaction.commandName).execute(interaction, client);
			} else {
		...
		} else if (interaction.isMessageComponent() || interaction.isAnySelectMenu() || interaction.isModalSubmit()) {
			...
			if (interaction.isButton() && !interaction.isAnySelectMenu()) {
				...
				for (const [, button] of Buttons.entries()) {
					if (
						...
						(!button.component.regex && button.component.id === interaction.customId) ||
						// Ou si le bouton accepte les Regex
						// et que l'id de l'interaction correspond au Regex du bouton
						(button.component.regex instanceof RegExp && button.component.regex.test(interaction.customId))
					) {
				...
				for (const [, modal] of Modals.entries()) {
					if (
						...
						(!modal.component.regex && modal.component.id === interaction.customId) ||
						// Ou si le formulaire accepte les Regex
						// et que l'id de l'interaction correspond au Regex du formulaire
						(modal.component.regex instanceof RegExp && modal.component.regex.test(interaction.customId))
					) {
				...
				for (const [, select_menu] of SelectMenus.entries()) {
					if (
						...
						(!select_menu.component.regex &&
							select_menu.component.id === (interaction as Discord.AnySelectMenuInteraction).customId) ||
						...
						(select_menu.component.regex instanceof RegExp &&
							select_menu.component.regex.test(
								(interaction as Discord.AnySelectMenuInteraction).customId
							))
					) {
			...
			Autocompletes.get(interaction.commandName)?.execute(interaction, client);
		}
