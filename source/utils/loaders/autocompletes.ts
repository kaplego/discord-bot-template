	...
	async function loadDir(dir: string) {
		...
		await asyncForEach(
			...
			async (file) => {
				...
				if (
					!fs.existsSync(`${SOURCE_DIR}/${AUTOCOMPLETE_FOLDER}/${dir}/${file.name.replace(/\.js$/, '.ts')}`)
				) {
					fs.rmSync(`${BUILD_DIR}/${AUTOCOMPLETE_FOLDER}/${dir}/${file.name}`);
					return;
				}
				...
				// Lire le fichier
				const filedata = (await import(`../../${AUTOCOMPLETE_FOLDER}/${dir}/${file.name}`))
