...
export const AllComponents = [] as (Bot.Button | Bot.Modal | Bot.AnySelectMenu)['component'][];
...
export async function loadComponents(): Promise<void> {
	...
	for (const component_type of ['button', 'modal', 'select_menu'] as const) {
		...
		if (!fs.existsSync(`${BUILD_DIR}/${COMPONENTS_FOLDER}/${component_type}`)) continue;
		logging.log(`▽ ${component_type}`.green);

		/** Charger un dossier de scripts */
		async function loadDir(dir: string) {
			// Lire le contenu du dossier `dir`
			const files = fs.readdirSync(`${BUILD_DIR}/${COMPONENTS_FOLDER}/${dir}`, {
				withFileTypes: true,
			});
			...
			await asyncForEach(files, async (file) => {
				...
				if (!fs.existsSync(`${SOURCE_DIR}/${COMPONENTS_FOLDER}/${dir}/${file.name.replace(/\.js$/, '.ts')}`)) {
					fs.rmSync(`${BUILD_DIR}/${COMPONENTS_FOLDER}/${dir}/${file.name}`);
					return;
				}
				...
				// Lire le fichier
				const filedata = (await import(`../../${COMPONENTS_FOLDER}/${dir}/${file.name}`)).default as
					| Bot.Button
					| Bot.Modal
					| Bot.AnySelectMenu;
				...
				switch (component_type) {
					case 'button':
						Buttons.set(filedata.component.id, filedata as Bot.Button);
						break;
					case 'modal':
						Modals.set(filedata.component.id, filedata as Bot.Modal);
						break;
					case 'select_menu':
						SelectMenus.set(filedata.component.id, filedata as Bot.AnySelectMenu);
						break;
					default:
	...
	[...Buttons.values(), ...Modals.values(), ...SelectMenus.values()].forEach((component) =>
