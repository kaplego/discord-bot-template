...
export async function loadEvents(client: Discord.Client): Promise<void> {
	...
	await asyncForEach(files, async (file) => {
		...
		if (!fs.existsSync(`${SOURCE_DIR}/${EVENTS_FOLDER}/${file.name.replace(/\.js$/, '.ts')}`)) {
		...
		// Lire le fichier
		const filedata = (await import(`../../${EVENTS_FOLDER}/${file.name}`)).default as Bot.Event<
		...
		client.on(filedata.name, (...args) => {
			logging.log(`[Event]       ${filedata.name.replace(/^./, (c) => c.toUpperCase())}`, ...args);
