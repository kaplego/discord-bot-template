...
export class LocalesManager {
	...
	public getOne(name: string, locale?: DiscordTypes.Locale): string {
		...
		const value = name.split('.').reduce((a, b) => (a !== null && b in a ? a[b] : null), localeData);

		if (value === null && locale !== this.defaultLocale) return this.getOne(name, this.defaultLocale);
	...
	public get(name: string):
		...
		this.locales.forEach((locale, localeID) => {
			const currentVal = name.split('.').reduce((a, b) => (a !== null && b in a ? a[b] : null), locale);
	 ...
	 */
	public command = (type: 'slash' | 'user' | 'message', commandName: string) =>
		({
			...this.baseObject(`commands.${type}.${commandName}`),
			option: (optionName: string) =>
				({
					...this.baseObject(`commands.${type}.${commandName}.options.${optionName}`),
