...
export default class LogManager {
	 ...
	 */
	public listeners: Map<keyof LogEvents, LogEvents[keyof LogEvents][]> = new Map([['logAdd', []]]);
	...
	constructor() {
		...
		this.logFile = `${moment(this.logDate).format('DD-MM-YYYY_HH-mm-ss')}.log`;

		this.checkLogFolder();

		if (fs.readdirSync(`./${LOG_FOLDER}`).length > 10) {
			fs.unlinkSync(`./${LOG_FOLDER}/${fs.readdirSync('./logs').filter((f) => f.endsWith('.log'))[0]}`);
		}

		fs.appendFileSync(`./${LOG_FOLDER}/${this.logFile}`, '========== BEGIN LOG ==========\n');
	}
	...
	private static removeANSI(message: string): string {
		return message.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/gu, '');
	}
	 ...
	 */
	private static logData(level: 'LOG' | 'INFO' | 'WARN' | 'ERROR' | 'CRITICAL', message: string): Omit<Log, 'args'> {
		const date = moment();
	 ...
	 */
	public addListener<E extends keyof LogEvents>(event: E, listener: LogEvents[E]): this {
	 ...
	 * @returns Le gestionnaire de logs.
	 */
	private callListener(event: keyof LogEvents, data: Parameters<LogEvents[keyof LogEvents]>): void {
		const listeners = this.listeners.get(event) || [];
